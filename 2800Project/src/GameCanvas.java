...
public class GameCanvas extends Canvas implements Runnable {
	...
	private BufferedImage backgroundSprite; // Background sprite image
	...
	public void run() {
		...
		while (true) {
			...
			if (System.currentTimeMillis() - timer > 1000) {
				timer += 1000;
				// System.out.println("Updates per second: " + updatesCounter + ", Frames per
				// second: " + framesCounter);
				// updatesCounter = 0;
				// framesCounter = 0;
			}
		}
	...
	// this function is called maximum times per second. Use it to draw the game's
	// graphics
	private void render() {
		// Gets the graphics of the buffer and enables anti-aliasing (looks nicer but
	...
	private void loadBackgroundSprite(String path) {
		try {
			backgroundSprite = ImageIO.read(getClass().getResourceAsStream(path));
		} catch (IOException e) {
			System.out.println("Error loading background sprite: " + e.getMessage());
			e.printStackTrace();
		}
	}
